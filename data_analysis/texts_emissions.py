...
def get_sectoral_info(sector: Sector, geo: Geo, df_wedges: pd.DataFrame,
    ...
    if sector == Sector.INDUSTRY:
        ...
        if geo == Geo.CZ:
            details = f'Emise v energetice pochází především ze spalování hnědého uhlí v elektrárnách ({total_electricity_str} milionů tun, resp. {total_electricity_percent_str} % celkových ročních emisí) a dále z tepláren ({_total("electricity-heat_CHP")} mil. tun, či {_percent("electricity-heat_CHP")} % celkových emisí ročně). Největším jednotlivým emitentem CO<sub>2</sub> je hnědouhelná elektrárna Počerady, která ročně vyprodukuje {_total("electricity-heat_pocerady")} mil. tun CO<sub>2</sub>, což je {_percent("electricity-heat_pocerady")} % celkových emisí České republiky. Pět největších českých hnědouhelných elektráren, Počerady, Ledvice, Prunéřov, Tušimice a Chvaletice, vyprodukují ročně zhruba tolik emisí CO<sub>2</sub> jako veškerá silniční doprava.'
        elif geo == Geo.SK:
