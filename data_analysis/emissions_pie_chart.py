...
def _get_wedge(
    wedge_def: dict, wedge_value: float, parent_id: Optional[str] = None
) -> Wedge:
    id = wedge_def["id"]
    ...
    return Wedge(
        ...
        value=wedge_value,
        parent_id=parent_id,
        label=wedge_def["label"],
        color=wedge_def["color"],
    )
...
def get_total_emissions_value(df_crf_and_allowances: pd.DataFrame) -> float:
    ...
    return (
        df_crf_and_allowances.loc["TOTX4_MEMO", "value"]
        + df_crf_and_allowances.loc["CRF1D1A", "value"]
    )
...
def get_emissions_wedges(
    ...
    all_codes = sum((wedge_def.get("codes", []) for wedge_def in definition), start=[])
...
def draw_emissions_pie_chart(
    ...
    # inner plot circle
    ax.pie(
        [wedge.value for wedge in inner_wedges],
        ...
        radius=(2 * inner_size),
        startangle=90,
        textprops={"fontsize": 12, "fontweight": "bold", "color": "#000000"},
        pctdistance=0.73,
        labeldistance=1.55,
